...
def cameraSeamBlend(IrIndv):
    ...
    cam_nums = np.shape(IrIndv)[-1]

    # Iterate through each camera
    for kk in range(cam_nums):
        K = IrIndv[:,:,kk]
...
def rectVideos(video_list, intrinsic_list, extrinsic_list, numFrames, grid, coords = 'local', origin = 'None', mType = 'CIRN'):
    ...
    for i in range(numFrames):
        ...
        merged = mergeRectify(frames, intrinsic_list, extrinsic_list, grid, coords, origin, mType)
