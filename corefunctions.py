...
def mergeRectify(input_frames, cameras, grid):
    ...
    extrinsics, intrinsics, and distorted images corresponding to each camera contained
    within the CameraData object. 
    ...
    Inputs:
        input_frames (list OR ndarray): 1xK list of paths to image files for each camera, 
...
def rectVideos(video_list, cameras, grid, numFrames):
    ...
    """
    This function performs image rectifications on video files, either single or multi- cam, 
...
class CameraData(object):
    ...
    def __init__(self, intrinsics, extrinsics, origin= 'None', coords = 'local', mType = 'CIRN', nc = 1):
